...
window.addEventListener("load", (ev) => {
    if(localStorage.getItem("username").trim != "") {
        ...
        let frm = new FormData()
        frm.append("uern", localStorage.getItem("username"))
        fetch("./utils/get-profile-img.php", {
            method: "POST",
            body: frm
        })
        .then((res) => res.text())
        .then((response) => {
            if(response.trim() != "null") {
                ...
                img.className = "h-12 w-12 object-cover mx-3 profile-img"
                img.src = response
                console.log(response)
                document.getElementById("m-row").append(img)
            } else {
                let img = document.createElement("img")
                img.className = "h-12 w-12 object-cover mx-3 profile-img"
